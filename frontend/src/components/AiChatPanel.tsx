...
// 작품의 AI 채팅 어시스턴트 — NovelDetailPage 전용이었던 로직을 그대로 컴포넌트로 추출해
// 회차 작성/수정 화면의 작업 패널에서도 동일한 로직·API·디자인으로 재사용한다.
export default function AiChatPanel({ novelId }: AiChatPanelProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      const answer = await sendChatMessage(novelId, trimmed);
      ...
      {hasIncompleteSummaries && (
        <div className="chat-warning">
          ...
          <button
            ...
            onClick={() => navigate(`/novels/${novelId}/episodes`)}
          >
            회차 관리로 이동
          </button>
        </div>
